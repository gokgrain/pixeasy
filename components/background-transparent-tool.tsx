...
export function BackgroundTransparentTool({ config }: { config: ToolConfig }) {
  ...
  useEffect(()=>{if(!pendingImage)return;const timer=window.setTimeout(()=>{void chooseFile(pendingImage.file)},0);return()=>window.clearTimeout(timer);// eslint-disable-next-line react-hooks/exhaustive-deps
  },[pendingImage]);
