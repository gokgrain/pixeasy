    ...
    <footer className="site-footer">
      <div className="wrap footer-inner">
        ...
        <nav className="footer-links" aria-label={messages.nav.footer}>
          <Link href={localePath(locale,"/blog")}>Blog</Link><Link href={localePath(locale, "/about")}>{messages.nav.about}</Link><Link href={localePath(locale, "/privacy")}>{messages.nav.privacy}</Link><Link href={localePath(locale, "/terms")}>{messages.nav.terms}</Link><a href="mailto:support@pixeasytools.com">{messages.nav.contact}</a>
        </nav>
      </div>
    </footer>
  );
