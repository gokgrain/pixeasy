...
const useCases: Record<Locale, Record<LegacyToolKind, string[]>> = {
  en: {
    ...
    resize: ["Remove unwanted space around an image before resizing.", "Reframe a profile photo, thumbnail, or banner.", "Crop the important area and resize it to exact pixel dimensions.", "Prepare square, portrait, or landscape images with ratio presets.", "Exclude unimportant areas before uploading an image."],
  ...
  ko: {
    ...
    resize: ["이미지 가장자리의 불필요한 여백을 제거하고 싶을 때", "프로필, 썸네일, 배너용으로 구도를 다시 잡고 싶을 때", "필요한 부분만 잘라낸 뒤 정확한 픽셀 크기로 조절하고 싶을 때", "정사각형, 세로형, 가로형 비율에 맞춰 이미지를 정리하고 싶을 때", "업로드용 이미지에서 중요하지 않은 부분을 제외하고 싶을 때"],
  ...
  ja: {
    ...
    resize: ["画像の端にある不要な余白を取り除きたいとき", "プロフィール、サムネイル、バナー用に構図を整えたいとき", "必要な部分だけを切り抜いて正確なピクセル寸法にしたいとき", "正方形、縦長、横長の比率に画像を合わせたいとき", "アップロード前に重要でない部分を除外したいとき"],
...
const content: Record<Locale, Record<LegacyToolKind, ToolSeoContentCore>> = {
  en: {
    ...
    resize: {
      steps: ["Upload a JPG, PNG, or WebP image.", "Optionally move or resize the crop frame, then set the output dimensions.", "Check the cropped and resized preview, then download it."],
      practical: [
      ...
      extraFaqs: [
        { question: "Can I crop an image before resizing it?", answer: "Yes. Select the area you want to keep first, then resize that cropped result to the dimensions you need." },
        { question: "Can I lock the crop aspect ratio?", answer: "Yes. Use presets such as 1:1, 4:3, or 16:9, keep the original ratio, or match the crop frame to the output dimensions." },
        { question: "Will resizing change the aspect ratio?", answer: "Width or height mode preserves the current crop ratio automatically. Exact mode uses both dimensions exactly and may change the proportions." },
      ],
  ...
  ko: {
    ...
    resize: {
      steps: ["JPG, PNG 또는 WebP 이미지를 선택합니다.", "필요하면 자르기 프레임을 조절한 뒤 출력 크기를 설정합니다.", "자르기와 크기 조절이 반영된 결과를 확인하고 다운로드합니다."],
      practical: [
      ...
      extraFaqs: [
        { question: "이미지를 자른 뒤 크기를 조절할 수 있나요?", answer: "네. 먼저 이미지에서 필요한 영역만 선택해 자른 뒤, 그 결과를 원하는 크기로 조절할 수 있습니다." },
        { question: "자르기 비율을 고정할 수 있나요?", answer: "네. 1:1, 4:3, 16:9 같은 비율 프리셋이나 원본 비율을 사용하고, 출력 크기 비율에 맞춰 자를 수도 있습니다." },
        { question: "크기 변경 시 가로세로 비율이 유지되나요?", answer: "너비 또는 높이 모드에서는 현재 자르기 영역의 비율이 자동으로 유지됩니다. 정확한 크기 모드는 두 값을 그대로 적용합니다." },
      ],
      related: ["jpg-png", "png-jpg", "grayscale", "invert"],
    },
    grayscale: {
  ...
  ja: {
    ...
    resize: {
      steps: ["JPG、PNG、WebP画像を選択します。", "必要に応じて切り抜き枠を調整し、出力サイズを設定します。", "切り抜きとサイズ変更を反映した結果を確認して保存します。"],
      practical: [
      ...
      extraFaqs: [
        { question: "画像を切り抜いてからサイズを変更できますか？", answer: "はい。必要な範囲を先に選択し、切り抜いた結果を希望するサイズへ変更できます。" },
        { question: "切り抜きのアスペクト比を固定できますか？", answer: "はい。1:1、4:3、16:9などのプリセット、元の比率、または出力サイズの比率を使用できます。" },
        { question: "サイズ変更で縦横比は維持されますか？", answer: "幅または高さモードでは現在の切り抜き範囲の比率を維持します。正確な寸法モードは入力した両方の値を使います。" },
      ],
      related: ["jpg-png", "png-jpg", "grayscale", "invert"],
    },
    grayscale: {
